...
for t=1:T_T
    ...
    err_n(t) = norm(err);
    err_s(t) = (1-ssim(double(R_trh),double(gtr_bin)))/2;
end
...
%% norm
[~,idx_mim] = min(err_n);
R_trh = uint8(Result>threshold_final_cut(idx_mim)*Trh);
err = abs(double(R_trh)-double(gtr_bin));
...
figure;
hold on
plot(threshold_final_cut,err_n,'ro')
plot(threshold_final_cut,err_s,'bx')
legend('L-2','SSIM')
grid
